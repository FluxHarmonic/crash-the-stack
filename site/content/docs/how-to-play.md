...
JACK IN starts a run: the layers of Gibson's defenses, each guarded by a board you clear to get through. FREE PLAY gives you either game on its own, in the hacker version or the classic one, plus a daily board of each kind (the same for everyone that day) and a place to enter a share code. Every board has a share code in its pause menu (Escape, or the MENU button): send it to a friend and they get the exact layout you played.
